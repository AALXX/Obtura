...
func EnsureNginxConfig(framework *Framework, projectPath string) error {
	needsNginx := framework.Name == "Static HTML/CSS" ||
	...
	if err := os.WriteFile(nginxPath, []byte(config), 0644); err != nil {
		return fmt.Errorf("failed to write nginx.conf: %w", err)
	}

	log.Printf("✅ Generated default nginx.conf for %s in %s", framework.Name, projectPath)
	return nil
}
